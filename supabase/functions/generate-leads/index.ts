...
Deno.serve(async (req) => {
  ...
  const functionName = 'generate-leads';
  ...
  try {
    ...
    // Check rate limit: 15 requests per minute for AI functions
    ...
    const leadsSchema = z.object({
      route: z.string()
        ...
        .max(500, 'Route description too long'),
      radius: z.number()
    ...
    try {
      ...
      validatedInput = leadsSchema.parse(requestBody);
    } catch (error) {
    ...
    const { route, radius } = validatedInput;
...
Format as JSON array with keys: name, address, industry, website, score, insights`,
    ...
    if (supabaseClient) {
      await supabaseClient.from('edge_function_logs').insert({
        ...
        status_code: statusCode,
        response_time_ms: Date.now() - startTime,
        metadata: { lead_count: leads.length }
      });
    }
